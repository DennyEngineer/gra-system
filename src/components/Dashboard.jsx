...
function Dashboard() {
  ...
  useEffect(() => {
    const regionalMetrics = data.regions;
    ...
    const avgTax = totalTax / totalTaxpayers;
    const avgCompliance = regionalMetrics.reduce((sum, r) => sum + r.complianceRate * r.taxpayers, 0) / totalTaxpayers;
  ...
  const exportToCSV = () => {
    ...
    const rows = filteredMetrics.map((region) => [
      `"${region.region}"`, // Wrap in quotes to handle commas
    ...
    const date = new Date().toISOString().slice(0, 10); // e.g., 2025-05-11
    link.setAttribute("href", URL.createObjectURL(blob));
    link.setAttribute("download", `taxpayer_data_${date}.csv`);
          ...
          <div className="mb-8 bg-gradient-to-br from-blue-900 to-teal-900 p-6 rounded-xl shadow-lg animate-fade-in">
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                <p className="text-sm text-gray-200 mt-1">Registered taxpayers across Ghana’s regions</p>
              </div>
                    ...
                    filteredMetrics.map((region) => (
                      <tr key={region.region} className="hover:bg-teal-50 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          <div className="text-sm text-gray-500">{((region.taxpayers / metrics.totalTaxpayers) * 100).toFixed(1)}%</div>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            <div className="w-20 bg-gray-200 rounded-full h-2 mr-2">
                              <div
                                className={`h-2 rounded-full ${region.complianceRate >= 0.8 ? "bg-teal-500" : region.complianceRate >= 0.5 ? "bg-yellow-500" : "bg-red-500"}`}
                                style={{ width: `${region.complianceRate * 100}%` }}
                              ></div>
                            </div>
                            <span className="text-sm font-medium">{(region.complianceRate * 100).toFixed(1)}%</span>
                          </div>
                        </td>
                      </tr>
                    ))
